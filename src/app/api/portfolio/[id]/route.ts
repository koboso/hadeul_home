...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  db.prepare(`
    ...
    SET category_id = ?, client = ?, title = ?, description = ?, detail = ?, image = ?, video = ?,
        tech_stack = ?, architecture = ?, target_device = ?, frame_enabled = ?, no_image = ?, updated_at = datetime('now')
    WHERE id = ?
  `).run(category_id, client, title, description, detail || "", image || "", video || "", tech_stack || "", architecture || "", target_device || "pc", frame_enabled ?? 1, no_image ?? 0, id);
